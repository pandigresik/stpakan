...
def main():
    ...
    if (loop == False):
        ...
        imgOriginalScene = imutils.transform (imgOriginalScene)
        imgOriginalScene,license = searchingOCR(imgOriginalScene,loop)
...
#coba menggunakan tesseract-ocr
def searchingOCR(imgOriginalScene,loop):
    licenses = ""
    if imgOriginalScene is None:                            # if image was not read successfully
        print("error: image not read from file \n")      # print error message to std out
        os.system("pause")                                  # pause so user can see error message
    ...
    # write the grayscale image to disk as a temporary file so we can
        ...
        cv2.imshow("Plat nomer",gray)
        licenses = pytesseract.image_to_string(Image.open(filename))
        os.remove(filename)
    return imgOriginalScene, licenses
###################################################################################################
