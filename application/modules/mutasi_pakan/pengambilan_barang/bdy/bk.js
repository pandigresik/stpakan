...
function show_detail(elm) {
    ...
    $(elm).next().find(
        'table tbody tr td input.berat-timbang:not([readonly]):first')
    .focus().select();
...
function selesai(elm) {
    ...
    var no_kavling = $('tr.tr-header[data-ke="'+detail_ke_detail+'"]').find('td.no-kavling').text();
...
function generate(e) {
    ...
    ajax_generate(e, tanggal_kebutuhan, no_penerimaan,no_referensi);

}

function ajax_generate(e, tanggal_kebutuhan,no_penerimaan,no_referensi) {
    $.ajax({
        type : "POST",
