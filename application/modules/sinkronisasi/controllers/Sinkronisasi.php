...
class Sinkronisasi extends MX_Controller
{
    ...
    public function index()
    {
        ...
        $data['data'] = $sinc;
        $data['limit'] = $limit;
        $data['list_option'] = [15, 50, 100, 150, 250, 500];
        $data['search'] = $search;
...
SQL;
    ...
    public function pushData($id)
    {
        ...
        //$id = 265;
        //$id2 = 265;
        $all_sinkron = $this->sinc->limit(1)->get_many_by(['id > '.$id.' and id_ref is null and tgl_sinkron is null']);

        if (!empty($all_sinkron)) {
            foreach ($all_sinkron as $data_sinkron) {
                //print_r($data_sinkron);
                $kirim = ['data_sinkron' => $data_sinkron, 'data_aplikasi' => []];
    ...
    public function lastIdSinkronLocal()
    {
        ...
        $lastId = $this->sinc->order_by('id', 'desc')->limit(1)->get_by(['asal != \''.$asal_farm.'\'', 'id_ref is not null']);
        if (!empty($lastId)) {
    ...
    public function updateInfo($waktuSinkron)
    {
        foreach ($waktuSinkron as $k => $w) {
            $this->sinc->update($k, ['tgl_sinkron' => $w]);
        }
    }
...
sql;
    ...
    public function sinkronUlang($id)
    {
        ...
        unset($conf['id'], $conf['tgl_sinkron']);
...
SQL;
    ...
    public function detailSinkron()
    {
        ...
        $dataSinkron = $this->dsinc->as_array()->get_many_by(['sinkronisasi' => $idSinkron]);
        $this->load->view('sinkronisasi/detail_sinkron', ['data' => $dataSinkron]);
    }

    private function buildSql($sql, $data_array = [])
    {
    ...
    {
        $list_aksi = $this->dsinc->order_by('no_urut', 'desc')->get_many_by(['sinkronisasi' => $idSinkronisasi]);

        $tmp = ['data_tabel' => []];
        $data_tabel = [];
        $aksi_arr = ['I' => 'insert', 'U' => 'update', 'D' => 'delete'];
        ...
        foreach ($list_aksi as $aksi) {
            ...
            if (in_array($data_tabel['aksi'], ['update', 'delete'])) {
                $data_tabel['_where'] = $where;
            } else {
                $data_tabel['_where'] = null;
            }
            if (in_array($data_tabel['aksi'], ['delete'])) {
                $content = [];
            } else {
