...
<table class="table table-bordered custom_table" >
	<thead>
			<tr>
				...
				<th class="ftl" colspan="<?php
					echo ($jml_pakan * 2) ?>">Stok Awal</th>
				<th colspan="<?php
					echo ($jml_pakan * 2) + 1 ?>">Penerimaan</th>
				<th colspan="<?php
					echo ($jml_pakan * 2) + 2 ?>">Pengeluaran</th>
				<th colspan="<?php
					echo ($jml_pakan * 2) ?>">Stok Akhir</th>
			</tr>
			<tr>
				<?php
...
foreach($farms as $f => $farm) {
	echo '<tr style="background-color:gray"><td colspan="4" style="text-align:left;font-size:14px; font-weight:bold"><strong>Farm ' . $namafarm[$f] . '(' . $f . ' ' . count($farm) . ' Kandang )</strong></td><td colspan="18"></td></tr>';
	foreach($farm as $kd => $data) {

		// cetak_r($farm, false);
		// cetak_r($data, false);
		// print_r($data['tgl_docin']);
		if(!isset($data['tgl_docin'])){
			echo '<tr style="background-color:#66cc00"><td colspan="4" style="text-align:left"><strong>' . $kd . '</strong></td><td colspan="18" ></td></tr>';
			echo '<tr><td colspan="4" style="text-align:left">Tidak ada data yang masuk</td><td colspan="18" ></td></tr>';
		}else{
			$tgl_docin = $data['tgl_docin'];
			$populasi_awal = $data['populasi_awal'];
			echo '<tr style="background-color:#66cc00"><td colspan="4" style="text-align:left"><strong>' . $kd . ' (' . $populasi_awal . ')' . '</strong></td><td colspan="18" ></td></tr>';
			...
			if ($showData) {
				...
				$total_keluar = array();
				foreach($kode_barang as $kd) {
					$stok_awal[$kd['kode_barang']] = array(
						'sak' => 0,
						'kg' => 0
					);
					$stok_akhir[$kd['kode_barang']] = array(
						'sak' => 0,
						'kg' => 0
					);
					$stok_awal_mingguan[$kd['kode_barang']] = array(
						'sak' => 0,
						'kg' => 0
					);
					$total_terima[$kd['kode_barang']] = array(
						'sak' => 0,
						'kg' => 0
					);
					$total_keluar[$kd['kode_barang']] = array(
						'sak' => 0,
						'kg' => 0
					);
					$total_terima_perminggu[$kd['kode_barang']] = array(
						'sak' => 0,
						'kg' => 0
					);
					$total_keluar_perminggu[$kd['kode_barang']] = array(
						'sak' => 0,
						'kg' => 0
					);
				}
				...
				while ($awal <= $akhir) {
					...
					foreach($kode_barang as $kd) {
						...
						$kkg_tmp[$_kb] = array();
						$ksak_tmp[$_kb] = array();
						$terimaKandang[$_kb] = array(
							'sak' => 0,
							'kg' => 0
						);
						$terimaGudang[$_kb] = array(
							'sak' => 0,
							'kg' => 0
						);
					}
	...
	echo '<tr><td colspan="4" style="text-align:left"></td><td colspan="18" ></td></tr>';
}
