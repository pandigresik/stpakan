	...
	<thead>
		...
		foreach($farms as $f => $farm){
			...
			if(empty($rtanggal)){
				...
				$jml_baris = date_diff($awal, $akhir)->format('%a');
				$jmlRekapnya = 2;
				$index_hari = date('w',strtotime($awal->format('Y-m-d')));
				$jmlRekapnya += ceil(($jml_baris + $index_hari)/7);
				$jml_baris += $jmlRekapnya;
				$showData = 1;
			}
			if($showData){
				$stok_awal = array();
				...
				while($awal <= $akhir){
					...
					foreach($kode_barang as $kd){
						$_kb = $kd['kode_barang'];
						echo '<td class="number">'.($stok_awal[$_kb]['sak'] > 0 ? formatAngka($stok_awal[$_kb]['sak'],0) : '-').'</td>';
						echo '<td class="number">'.($stok_awal[$_kb]['kg'] > 0 ? formatAngka($stok_awal[$_kb]['kg'],3) : '-').'</td>';
					/* nitip looping */
						$kkg_tmp[$_kb] = array();
						$ksak_tmp[$_kb] = array();
