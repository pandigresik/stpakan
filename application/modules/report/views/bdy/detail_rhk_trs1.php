<table class="table table-bordered custom_table" >
	<thead>
			<tr>
			  ...
			  <th class="" rowspan="3">Populasi <br> Awal</th>
				...
				<th class="screen_2" colspan="7">Rasio Performa thd. Standar</th>
				<th class="screen_3" colspan="5">Pertanggungjawaban Retur</th>
				<th class="screen_3" colspan="14">Kontrol Pakan</th>
				<th class="screen_3" rowspan="3">Catatan</th>
			</tr>
			<tr>
				<th class="screen_2" colspan="3">Deplesi</th>
				<th class="screen_2" rowspan="2">Pop. <br /> (ekor)</th>
				<th class="screen_2" rowspan="2">Panen <br /> (ekor)</th>
	...
	foreach($farms as $f => $farm){
		echo '<tr><td colspan="4">Farm '.$f.'</td></tr>';
		...
		if(!empty($rhk)){
			...
			//		$span_bookmark = '<span class="bookmark">Minggu '.((int)$r['hari']/7 - 1).'</span>';
				...
				if(!$_counter){
					...
					echo '<td rowspan="'.$_totalBaris.'" class="text-center tanggal">'.$populasi_awal['stok_awal'].'</td>';
					$_counter++;
				}
				echo '<td rowspan="'.$t_rowspan.'" class="screen_2 text-center tanggal">'.tglIndonesia($tgl,'-',' ').$span_bookmark.'</td>';
				echo '<td rowspan="'.$t_rowspan.'"  class="screen_2 number">'.$r['hari'].'</td>';
				...
				echo '<td style=\'mso-number-format:"#.##0"\' rowspan="'.$t_rowspan.'"  class="screen_2 number">'.$c_panen.'</td>';
	...
	<div class="btn prev slider-table" data-current="2" data-min="2" data-max="3" onclick="Rhk.next(this)"> <i class="glyphicon glyphicon-chevron-right"></i> </div>
	<div class="btn next slider-table" data-current="2" data-min="2" data-max="3" onclick="Rhk.prev(this)"> <i class="glyphicon glyphicon-chevron-left"></i> </div>
