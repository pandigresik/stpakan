...
class M_realisasi_panen extends CI_Model{
	...
	public function get_data_do($noreg = NULL){
		...
		$sql = <<<QUERY
		select plg.NAMA_PELANGGAN, do.* 
		,CASE WHEN substring(DO.NO_DO,0,4) = 'BDY' THEN 0 ELSE 1 END LOCK_TONASE_DO
		,CASE WHEN substring(DO.NO_DO,0,4) = 'BDY' THEN 3000 ELSE 2500 END BERAT_RIT
		,CASE WHEN substring(DO.NO_DO,0,4) = 'BDY' THEN 30000 ELSE 2500 END MAX_RIT
