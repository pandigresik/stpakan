...
echo '
	...
	<table width="100%" style="font-family:Arial;font-size:6px;" id="header-barang">
		<tr>
			<td colspan="4" align="center" style="font-size:8px;"><b>Daftar Pengiriman Barang</b></td>
		</tr>
		<tr>
...
foreach ($items as $key => $value) {
    // if ($value['keterangan'] == 1 && $value['penerimaan_kandang'] == 0) {
    if ($value ['keterangan'] == 2) {
        $jumlah = $value ['tmp_jumlah'] + $value ['jumlah'];
        $outstanding = $value ['jml_order_outstanding'];#$value ['kebutuhan_pakan'] - $jumlah;
        echo '<tr>
					<td>' . $value ['kode_kandang'] . '</td>
					<td>' . $value ['jenis_kelamin'] . '</td>
					<td>' . $value ['id_kavling'] . '</td>
					...
					<td class="number">' . $outstanding . '</td>
					<td class="number">' . $jumlah . '</td>
					<td class="number">' . $value ['berat'] . '</td>
					<td class="number">' . $value ['tmp_jumlah'] . '</td>
					<td></td>
					</tr>';
    }
}
