...
<table class="table table-bordered table-striped" id="history-pallet">
    <thead>
        <tr>
            <th class="text-center col-md-2">ID Pallet</th>
    ...
    <tbody>
        <?php foreach ($pallet as $key => $value) { ?>
            ...
            <?php
                // if($key == (count($pallet)-1)){
                if($key == 0){
                    $status_label = '<select data-status="'.$value['STATUS_PALLET'].'" onchange="dialog_status_pallet(this)" id="select_status" name="select_status" class="form-control">';
            ...
            ?>
            <tr>
            <td class='text-center col-md-2 id_pallet'><?php echo $value['KODE_PALLET']; ?></td>
