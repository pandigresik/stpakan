...
class Main extends MY_Controller
{
    ...
    public function cek_syarat_tutup_budget()
    {
    ...
    public function load_budget_glangsing()
    {
    ...
    public function cek_status_siklus()
    {
        $user_level = $this->session->userdata('level_user');
        ...
        if (in_array($status_siklus, ['', 'D'])) {
            /** ambil nilai dari database config general */
            $sqlFarm = $this->db->select(['kode_farm'])->where(['kode_siklus' => $kode_siklus])->get_compiled_select('m_periode');
            $timelinePengajuan = $this->db->where(['STATUS' => 1, 'KODE_CONFIG' => '_min_pengajuan'])->where('kode_farm in ('.$sqlFarm.')')->get('sys_config_general')->row();
    ...
    public function kepalafarm($kode_siklus, $status_siklus, $terlambatTimeline)
    {
        $tutup_bugdet = 'none';
        $simpan = '';
        $rilis = '';
        ...
        if ('A' == $status_siklus) {
            ...
            $rilis = 'none';
            $field = 'read';
        } elseif ('' == $status_siklus || 'D' == $status_siklus || 'RJ' == $status_siklus) {
    ...
    public function kadeptBdy($kode_siklus, $status_siklus, $terlambatTimeline)
    {
        $tutup_bugdet = 'none';
        $review = '';
        $reject = '';
    ...
    public function kadept($kode_siklus, $status_siklus, $terlambatTimeline)
    {
        $tutup_bugdet = 'none';
        $review = '';
        $reject = '';
    ...
    public function cek_status_siklus_home()
    {
    ...
    public function cekKontrolTutupBudget()
    {
        ...
        /** cek permintaan sak */
        $mintaSak = $this->cekPermintaanSak($kode_farm, $periode)->result_array();
        if (empty($mintaSak)) {
            ...
            if (!empty($stokGlangsing)) {
                $tmpMessage = [];
                foreach ($stokGlangsing as $sg) {
                    if (!empty($sg['kontrol'])) {
                        $result['status'] = 0;
                        array_push($tmpMessage, 'Kode barang '.$sg['kode_barang'].' belum proses '.$sg['jml_stok']);
                    }
                }

                if (!empty($tmpMessage)) {
                    $result['message'] = 'Proses tutup budget tidak dapat dilanjutkan. Terdapat glangsing yang belum diproses';
                    $result['message'] .= '<div>'.implode('</div><div>', $tmpMessage).'</div>';
                }
            }
        } else {
            $listPermintaanBelumProses = array_column($mintaSak, 'no_ppsk');
            $result['status'] = 0;
            $result['message'] = 'Proses tutup budget tidak dapat dilajutkan. Terdapat permintaan sak yang belum di-Approve';
            $result['message'] .= '<div>'.json_encode($listPermintaanBelumProses).'</div>';
        }
...
SQL;
        ...
        return ['status' => $terlambat, 'tgl_closing' => $result->tgl_closing];
    }
}
