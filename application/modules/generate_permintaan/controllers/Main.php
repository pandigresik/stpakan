...
class Main extends MY_Controller {
    ...
    public function get_data_order_kandang() {
        return $this->m_main->get_data_order_kandang($this->_farm);
    }
    ...
    public function get_data_generate() {
        $kode_farm = $this->_farm; // $this->input->post ( 'kode_farm' );
        $tanggal_kebutuhan_awal = $this->input->post('tanggal_kebutuhan_awal');
        $tanggal_kebutuhan_awal = date('Y-m-d', strtotime(convert_month($tanggal_kebutuhan_awal, 2)));
        $tanggal_kebutuhan_akhir = $this->input->post('tanggal_kebutuhan_akhir');
        $tanggal_kebutuhan_akhir = date('Y-m-d', strtotime(convert_month($tanggal_kebutuhan_akhir, 2)));
        return $this->m_main->group_daftar_barang($kode_farm, $tanggal_kebutuhan_awal, $tanggal_kebutuhan_akhir);
    }

    public function generate() {
        $data ['barang'] = $this->get_data_generate();
    ...
    public function simpan_generate_permintaan() {
        ...
        $tanggal_kebutuhan_akhir = $this->input->post('tanggal_kebutuhan_akhir');
        $tanggal_kebutuhan_akhir = date('Y-m-d', strtotime(convert_month($tanggal_kebutuhan_akhir, 2)));
        $result = $this->m_main->simpan_generate_permintaan($kode_farm, $tanggal_kirim, $tanggal_kebutuhan_awal, $tanggal_kebutuhan_akhir, $this->_user);
        echo json_encode($result);
    }
