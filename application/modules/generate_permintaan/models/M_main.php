...
class M_main extends MY_Model {
    ...
    public function get_data_order_kandang($kode_farm) {
        $query = <<<QUERY

       		select distinct
				ld.KODE_FARM kode_farm
				, REPLACE(CONVERT(VARCHAR(10),ld.TGL_KIRIM,105),'-',' ') tgl_kirim
				, REPLACE(CONVERT(VARCHAR(10),ld.TGL_KEB_AWAL,105),'-',' ') tgl_keb_awal
				, REPLACE(CONVERT(VARCHAR(10),ld.TGL_KEB_AKHIR,105),'-',' ') tgl_keb_akhir
			...
			left join ORDER_KANDANG ok on ok.KODE_FARM = ld.KODE_FARM or ok.TGL_KIRIM = ld.TGL_KIRIM
			left join ORDER_KANDANG_D okd on ok.KODE_FARM = okd.KODE_FARM and ok.NO_ORDER = okd.NO_ORDER and okd.NO_REG = le.NO_REG
			left join ORDER_KANDANG_E oke on oke.KODE_BARANG = le.KODE_BARANG and oke.NO_REG = le.NO_REG and oke.JENIS_KELAMIN = le.JENIS_KELAMIN and oke.TGL_KEBUTUHAN = le.TGL_KEBUTUHAN --and oke.JML_ORDER IS NULL
			WHERE ld.KODE_FARM = '$kode_farm'
			and l.TGL_APPROVE1 is not null
			GROUP BY ld.KODE_FARM
					, ld.TGL_KIRIM
					, ld.TGL_KEB_AWAL
					, ld.TGL_KEB_AKHIR
QUERY;
...
QUERY;
    ...
    public function get_daftar_barang($kode_farm = NULL, $tanggal_kebutuhan_awal = NULL, $tanggal_kebutuhan_akhir = NULL) {
        $query = <<<QUERY
       		...
       		EXEC DAFTAR_BARANG_GENERATE_PERMINTAAN 
			   '$kode_farm',
...
QUERY;
    ...
    public function get_tambah_daftar_barang($kode_barang = NULL, $kode_farm = NULL, $tanggal_kebutuhan_awal = NULL, $tanggal_kebutuhan_akhir = NULL) {
        $query = <<<QUERY
       		...
       		EXEC TAMBAH_BARANG_GENERATE_PERMINTAAN 
			   '$kode_barang',
			   '$kode_farm',
			   '$tanggal_kebutuhan_awal',
			   '$tanggal_kebutuhan_akhir'
...
QUERY;
    ...
    public function simpan_release($data, $user) {
        ...
        $query = <<<QUERY
			...
			EXEC RELEASE_GENERATE_PERMINTAAN_BARU
				'$kode_farm',
				'$kode_barang',
				'$no_order',
				$jml_pp,
...
QUERY;
    ...
    public function simpan_generate_permintaan($kode_farm, $tanggal_kirim, $tanggal_kebutuhan_awal, $tanggal_kebutuhan_akhir, $user) {
        $query = <<<QUERY
			...
			EXEC SIMPAN_GENERATE_PERMINTAAN 
			   '$kode_farm',
			   '$tanggal_kirim',
			   '$tanggal_kebutuhan_awal',
			   '$tanggal_kebutuhan_akhir',
