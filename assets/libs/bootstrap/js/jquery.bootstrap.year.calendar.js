 ...
 */
 ...
 { 
    ...
    $('.jqyc-year-chooser').find('.col').each(function(i,v){
        $(v).removeClass('col');
        $(v).addClass(xar[i]);
    });
 }
 ...
 function dblclickmodalform()
 {
    $('.jqyc-not-empty-td').on('dblclick', function(){
        var d = $(this).attr('data-day-of-month');
        var m = $(this).attr('data-month');
        var y = $(this).attr('data-year');      

        var formattext = d + '-' + m + '-' + y;
        ...
        $.ajax({
            ...
            url : "master/kalenderlibur/check"
        }).done(function(data) {

            if (data.status == 'satu') {
                $('input[name=hstatus]').val('ubah');

                $('input[name=dari]').val(formattext);
                $('input[name=ddari]').val(formattext);
                $('input[name=dari]').prop('disabled', true);

                $('input[name=sampai]').val(formattext);
                $('input[name=dsampai]').val(formattext);
                $('input[name=sampai]').prop('disabled', false);

                $('textarea[name=keterangan]').val(data.fetch.KETERANGAN);
            } else {
                $('input[name=hstatus]').val('baru');

                $('input[name=dari]').val(formattext);
                $('input[name=ddari]').val(formattext);
                $('input[name=dari]').prop('disabled', false);
            ...
            $('#modal_kalenderlibur').modal("show");
        });
    });
 }


 function hoverTooltipDetil() {

 }


 function markToday() {
    var fullDate = new Date(), y = fullDate.getFullYear(), m = fullDate.getMonth() + 1, d = fullDate.getDate(); 
    var formatdate = [y,m,d].join("-");

    $('.jqyc-not-empty-td').each(function(i,v) {
        var din = parseInt($(v).attr('data-day-of-month')); 
        var min = parseInt($(v).attr('data-month'));
        var yin = $(v).attr('data-year');

        var formatdatein = [yin,min,din].join("-");

        if (formatdate == formatdatein) {
            $(v).css({'border' : '2px solid'});
        };        
    });
 }
 ...
 function drawday(ds) {
    $('.jqyc-not-empty-td').each(function(i,v){
        var d = parseInt($(v).attr('data-day-of-month')); 
        var m = parseInt($(v).attr('data-month'));
        var y = $(v).attr('data-year');
        ...
        $.each(ds, function(i,v){
            var perdate = new Date(v.startDate);
            ...
            dsarr.push(formatpush);
            dsname.push(v.name);
        });

        if ($.inArray(formatdate, dsarr) !== -1) {
            var ctlawas = $(v).html();

            $(v).html('<a style="text-decoration: none; color: red; font-weight: bold;" href="javascript:void(0)">'+ ctlawas +'</a>');
            ...
            $(v).find('a').popover({
                ...
                placement : 'top',
                content : function () {
                    return '<div class="hover-hovercard"><p style="color: black;">'+ dsname[$.inArray(formatdate, dsarr)] +'</p></div>';
                }
            }).mouseover(function(){
                var _this = this;
                $(this).popover('show');
                $(this).siblings('.popover').mouseout(function(){
                    $(_this).popover('hide');
                });
            }).mouseout(function(){
                var _this = this;
...
function getallkalenderlibur()
{
    /*$('.jqyc-header').click(function(){ 
        ...
        $.ajax({
            ...
            dataType : 'json',
            data : {
                ym : $(this).parent().attr('data-year') +'-'+ $(this).parent().attr('data-month')
            },
            url : baseUrl + "/server/getallkalenderlibur.php"
        }).done(function(data) {
            if (data.length > 0) {
                var mont = ["Januari", "Februari", "Maret", "April", "Mei", "Juni", "Juli", "Agustus", "September", "OKtober", "Nopember", "Desember"];

                var titlex = 'Daftar libur ' + mont[parseInt(jqycheader.parent().attr('data-month'))-1] +' '+ jqycheader.parent().attr('data-year');

                var htmlstr = "";
                $.each(data, function(i,v){
                    htmlstr += "<p>" + v.TANGGAL + ' : ' + v.KETERANGAN  + "</p>";
                });

                $('#modal_listlibur').find('.modal-header').find('.modal-title').html(titlex);

                $('#modal_listlibur').find('.modal-body').html(htmlstr);
                $('#modal_listlibur').modal("show");
            }
        });
...
function tooltipgetallkalenderlibur()
{
    $('.jqyc-header').each(function(i,v){
        var textheader = $(v).html();
        ...
        $.ajax({
            ...
            dataType : 'json',
            data : {
                ym : jqycheader.parent().attr('data-year') +'-'+ jqycheader.parent().attr('data-month')
            },
            url : "master/kalenderlibur/getallkalenderlibur"
        }).done(function(data) {
            if (data.length > 0) {
                var mont = ["Januari", "Februari", "Maret", "April", "Mei", "Juni", "Juli", "Agustus", "September", "OKtober", "Nopember", "Desember"];
                ...
                $.each(data, function(i,v){
                    htmltitle += '<p>' + formatDMY(v.TANGGAL) + ' : ' + v.KETERANGAN + '</p>';
                });

                var htmlstr = '<a href="javascript:void(0)" data-html="true" data-toggle="popover" data-trigger="hover" title="'+ titlex +'" data-content="'+ htmltitle +'">'+ textheader +'</a>';

                jqycheader.html(htmlstr);

                jqycheader.find('a').popover();
            }
        });
    });
}
...
(function ($) {
    ...
    $.fn.calendar = function (options, ds) {
        $calendar = this;
        if (methods[options]) {
