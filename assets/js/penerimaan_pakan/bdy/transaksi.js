...
function reset_detail_do(){
    ...
    $('#nopol-terima').data('keterangan','');
}

function cek_tara_pallet(callback) {
    $.ajax({
        type : "POST",
        url : "penerimaan_pakan/transaksi/cek_tara_pallet",
        dataType : 'json',
        success : function(data) {
...
function verifikasi_do(callback) {
    ...
    $.ajax({        
        type : "POST",
        beforeSend : function(){
            messageBox('Informasi','Harap tunggu sedang proses verifikasi DO ');
        },
...
function data_sj() {
    ...
    var _kuantitas_kg = [];
    var _kuantitas_zak = [];
    var _tgl_verifikasi_do = [];
    if(daftar_do_dan_sj){
        $.each(daftar_do_dan_sj, function(key, value){
            _do.push(key);
            _spm.push(value['no_spm']);
            _sj.push(value['no_sj']);
            _tanggal_sj.push(value['tanggal_sj']);
            _kuantitas_kg.push(value['kuantitas_kg']);
            _kuantitas_zak.push(value['kuantitas_zak']);
            _tgl_verifikasi_do.push(value['tanggal_verifikasi_do']);
        });
    }
    return {
        'do_params' : _do,
        'spm_params' : _spm,
        'sj_params' : _sj,
        'tanggal_sj_params' : _tanggal_sj,
        'kuantitas_kg_params' : _kuantitas_kg,
        'kuantitas_zak_params' : _kuantitas_zak,
        'tgl_verifikasi_do_params' : _tgl_verifikasi_do
    };
...
function daftar_do_dan_sj() {  
    ...
    var _html = '<table class="table table-bordered">';
        ...
        $.each(daftar_do_dan_sj, function(key, value){
            ...
            _html += '<td>'+Config._tanggalLocal(Config._getDateStr(new Date(value.tanggal_terima)),'-',' ')+'</td>';
...
function input_do() {
    reset_detail_do();
    cek_tara_pallet(function(result) {
        if (result.success == true) {
            verifikasi_do(function(data) {
                var nomor_do = $.trim($('#nomor-do').val());
                if(!data.status){
                    $('#nomor-do').parent().append('<span class="do-not-valid">'+data.message+'</span>');
                }
                else{
                    global_no_penerimaan = 0;
                    if(data.content.no_penerimaan){
                    	global_no_penerimaan = 1;
                    }                    
                    if(data.content.list_do){
                        set_all_view(data.content.list_do);
                    }
                    else{                        
                        if(data.content.validasi_tanggal_kirim == 0){
                                messageBox('Peringatan','<div class="text-center">Penerimaan terlambat, melebihi target tanggal kirim ('+Config._tanggalLocal(Config._getDateStr(new Date(data.content.tanggal_kirim)),'-',' ')+') harap melakukan koordinasi lebih lanjut.</div>');
                            }

                            if(data.all_data){                             
                                var all_data = JSON.parse(data.content.all_data);
                                set_all_local_storage(all_data);                            
                                daftar_do_dan_sj();
                                lanjut();

                            }
                            else{
                                set_daftar_do_dan_sj(data.content, nomor_do);                        
                                daftar_do_dan_sj();
                                lanjut();                                
                            }                
            ...
            messageBox('Peringatan','<div class="text-center">Harap melakukan tara pallet dan hand pallet <br /> terlebih dahulu, sebelum melakukan <br /> penerimaan pertama</div>');
        }
    });
}
...
function lanjut() {
    ...
    if(_params['do_params'].length > 0){
        ...
        $.each(daftar_do_dan_sj, function(key, value){
            ...
            $('#nopol-terima').val(value.nopol_terima);
            $('#label-nopol-terima').text(value.nopol_terima);
            $('#sopir').val(value.sopir);
            $('#label-sopir').text(value.sopir);
            $('div.detail-do label').removeClass('grey-label');
            $('#div-tanggal-terima').removeClass('hide');
            $('#panel-penerimaan-pakan').removeClass('hide');
            if(empty(value.sopir)){
                $('#sopir').removeClass('hide');    
                $('#label-sopir').remove();
            }
            if(empty(value.nopol_terima)){
                $('#nopol-terima').removeClass('hide');    
                $('#label-nopol-terima').remove();
            }
            $('#sopir').trigger('change');
        ...
        if(localStorage.getItem('nopol_sopir_'+kode_farm+'_'+global_no_penerimaan)){
            ...
            $('#label-nopol-terima').text(nopol_sopir.nopol_terima);
            $('#label-sopir').text(nopol_sopir.sopir);
            verifikasi();
        }
        else{
...
function detail_penimbangan_pakan(elm){
    ...
    if(penimbangan_pakan && penimbangan_pakan[kode_pakan]){
        ...
        $.each(penimbangan_pakan[kode_pakan]['detail'], function(key, value){
            ...
            _html += '<td class="selesai" data-selesai="1"><input type="checkbox" checked disabled /></td>';            
            _html += '</tr>';
                    ...
                    if(!empty(data)){
                        ...
                        if(total_timbangan>jml_sj){
                            ...
                            if($('tr.tr-detail[data-ke="'+data_ke_detail+'"] tr.tr-sub-detail-pakan[data-ke="'+data_ke_detail_pakan+'"]').length == 0){
                                var no_kavling = $('tr.tr-detail[data-ke="'+data_ke_detail+'"] tr.tr-detail-pakan[data-ke="'+data_ke_detail_pakan+'"] td.no-kavling span:first').text();
                                cek_maks_pallet(no_kavling, function(data_pallet){
                                    ...
                                    if((jml_sak+sak_tersimpan)>maks_pallet){
                                        ...
                                        if($(elm).hasClass('berat-timbang') && data.KONFIRMASI_SAK){
                                            $(elm).removeAttr('readonly');
                                            $(elm).attr('disabled', true);
                                            var konfirmasi = 0;
                                            var html = '';
                                            ...
                                            html += '<label class="col-md-12 text-center" for="">Masukkan jumlah aktual sak</label>';
                                            ...
                                            box.bind('hidden.bs.modal', function() {
                                                if(konfirmasi == 1){
                                                    ...
                                                    $('tr.tr-detail[data-ke="'+data_ke_detail+'"] tr.tr-detail-pakan[data-ke="'+data_ke_detail_pakan+'"] td.berat-bersih').text(berat_bersih);
                                                    $('tr.tr-detail[data-ke="'+data_ke_detail+'"] tr.tr-detail-pakan[data-ke="'+data_ke_detail_pakan+'"] td.timbangan-sak').text(jml_sak);
                                                    /** click detail untuk mengaktifkan tombol selesai */
                                                    $.each($('tr.tr-detail[data-ke="'+data_ke_detail+'"] tr.tr-sub-detail-pakan[data-ke="'+data_ke_detail_pakan+'"] table.tbl-detail-kandang tbody tr.tr-detail-kandang'), function(){
                                                        $(this).find('td.checkbox-kandang input.checkbox-kandang').click();
                                                    });
                                                    /** autofocus ke barcode pallet */
                                                    $(elm).closest('tr').find('td.selesai input').focus();
                                                }
                                                else{
...
function validasi_selesai(elm){
    ...
    if(timbangan == sum_jml_aktual){
       ...
       $('tr.tr-detail[data-ke="'+data_ke_detail+'"] tr.tr-detail-pakan[data-ke="'+data_ke_detail_pakan+'"] td.selesai input').prop('readonly',0);
    }
    else{
       //$('tr.tr-detail[data-ke="'+data_ke_detail+'"] tr.tr-detail-pakan[data-ke="'+data_ke_detail_pakan+'"] td.selesai button').attr('disabled', true);
       $('tr.tr-detail[data-ke="'+data_ke_detail+'"] tr.tr-detail-pakan[data-ke="'+data_ke_detail_pakan+'"] td.selesai input').prop('readonly',1);
    }
}
...
function selesai(elm){
    ...
    var no_kavling = $('tr.tr-detail[data-ke="'+data_ke_tr_detail+'"] tr.tr-detail-pakan[data-ke="'+data_ke_tr_detail_pakan+'"] td.no-kavling span:first').text();
   ...
   // $(elm).replaceWith('<input type="checkbox" checked />');
}
...
function get_data_surat_jalan(no_op, nomor_do, callback) {
    $.ajax({
        type : "POST",
...
function verifikasi(){
    $('button.btn-resave').addClass('hide');
    $('#penimbangan-pakan').html('');
    var nomor_do = $('#nomor-do').val();
    var no_penerimaan = $('#nomor-do').attr('data-no-penerimaan');
    $('#nopol-terima').attr('readonly', true);
    ...
    $('#btn-verifikasi').attr('disabled', true);
    set_nopol_sopir();   
    /** jika belum ada no_penerimaan berarti belum dilakukan penerimaan pakan, cek apakah nopol kirim = nopol terima */
    if(empty(no_penerimaan)){
        var _nopol_kirim = $('#nopol-kirim').text().replace(/[\s+-]/g,'');
        var _nopol_terima = $.trim($('#nopol-terima').val());
        
        if(_nopol_kirim != _nopol_terima){
            $('#nopol-terima').addClass('input_error');
            var _content = ['<div class="text-center">',						
            '<div class="col-md-12">Masukkan alasan Nopol Terima <br />berbeda dengan Nopol Kirim</div>',
                '<div class="col-md-12">',
                    '<textarea name="keterangan_beda_nopol" maxlength="200" class="col-md-12"></textarea>',
                '</div>',												
            '</div>'];
            var _options = {
                closeButton: false,
                ...
                callback : function(result){                    
                    var _alasan = $('.bootbox-body').find('textarea[name=keterangan_beda_nopol]').val();                    
                    var _result = false;
                    if($.trim(_alasan).length){                                                 
                        $('#nopol-terima').data('keterangan',_alasan);
                        detail_do();                        
                        _result = true;
                    }                    
                    return _result;
    ...
    if(_params['do_params'].length > 0){
        $.ajax({
            ...
            success : function(data) {
                ...
                /* tampilkan langsung */
                $('#tbl-detail-penerimaan tbody>tr.tr-header').each(function(){
                    $(this).trigger('dblclick');
                })

                if(global_no_penerimaan == 0 && cek_simpan() == 0){               
                    $('button.btn-resave').removeClass('hide');
                }
            }
        });
    }
...
function set_ganti_kavling(elm, data_ke_detail, data_ke_detail_pakan){
    ...
    var data_berat_pallet = $('tr.tr-detail[data-ke="'+data_ke_detail+'"] tr.tr-detail-pakan[data-ke="'+data_ke_detail_pakan+'"] td.no-kavling').attr('data-berat-pallet');
    var data_berat_hand_pallet = $('tr.tr-detail[data-ke="'+data_ke_detail+'"] tr.tr-detail-pakan[data-ke="'+data_ke_detail_pakan+'"] td.no-kavling').attr('data-berat-hand-pallet');
    var no_kavling = (!elm) ? data_no_kavling : $(elm).find('td.gk_no_kavling').text();
    var _berat = '';
    if(data_berat_pallet && data_berat_hand_pallet){
        _berat = parseFloat(data_berat_pallet)+parseFloat(data_berat_hand_pallet);
    }
    var berat = (!elm) ? _berat : $(elm).find('td.gk_berat').text();
    var sak = (!elm) ? '0' : $(elm).find('td.gk_sak').text();
    var status = (!elm) ? 1 : 0;
    $('tr.tr-detail[data-ke="'+data_ke_detail+'"] tr.tr-detail-pakan[data-ke="'+data_ke_detail_pakan+'"] td.no-kavling span:last').attr('data-status',status);
    $('tr.tr-detail[data-ke="'+data_ke_detail+'"] tr.tr-detail-pakan[data-ke="'+data_ke_detail_pakan+'"] td.no-kavling span:first').text(no_kavling);
    ...
    if(!elm){
        autoplus(data_ke_detail, data_ke_detail_pakan);
    }
...
function build_save_params(){
    ...
    var _data_kavling = JSON.parse(localStorage.getItem('data_kavling_'+kode_farm+'_0'));
    var data_timbang = {
      'daftar_do_dan_sj' : _data_daftar_do_dan_sj,
      'nopol_sopir' : _data_nopol_sopir,
      'penimbangan_pakan' : _data_penimbangan_pakan,
      'pakan_rusak_hilang' : _data_pakan_rusak_hilang,
      'data_kavling' : _data_kavling
    }
    ...
    var _penerimaan = {
        ...
        'tanggal_sj' : _data_sj['tanggal_sj_params'],
        'kuantitas_kg' : _data_sj['kuantitas_kg_params'],
        'kuantitas_zak' : _data_sj['kuantitas_zak_params'],
        'tgl_verifikasi_do' : _data_sj['tgl_verifikasi_do_params']
    }
...
function simpan(){
    ...
    var nomor_do = $('div#table-daftar-do-sj table tbody tr:first td:first').text();
    if(status_simpan == 0){
        $.ajax({
            type : 'POST',
            dataType : 'json',
            ...
            data : {
                ...
                nomor_do : nomor_do,
                keterangan_nopol : $('#nopol-terima').data('keterangan')
            }
        }).done(function(data) {
            if(data.result == 1){
                localStorage.removeItem('daftar_do_dan_sj_'+kode_farm+'_0');
...
function save_data_timbang(){
    var nomor_do = $('div#table-daftar-do-sj table tbody tr:first td:first').text();
    var data = build_save_params();
    $.ajax({
        type : "POST",
        url : "penerimaan_pakan/transaksi/save_data_timbang",
        data : {
            all_data : JSON.stringify(data.all_data),
            nomor_do : nomor_do
        },
...
function set_all_local_storage(data){
    localStorage.setItem('daftar_do_dan_sj_'+kode_farm+'_0',JSON.stringify(data.daftar_do_dan_sj));
    localStorage.setItem('data_kavling_'+kode_farm+'_0',JSON.stringify(data.data_kavling));
    localStorage.setItem('nopol_sopir_'+kode_farm+'_0',JSON.stringify(data.nopol_sopir));
    localStorage.setItem('pakan_rusak_hilang_'+kode_farm+'_0',JSON.stringify(data.pakan_rusak_hilang));
    localStorage.setItem('penimbangan_pakan_'+kode_farm+'_0',JSON.stringify(data.penimbangan_pakan));
}
...
function ganti_hand_pallet(elm){
    ...
    if(status == 1){
        ...
        $.ajax({
            type : 'POST',
            dataType : 'html',
            url : "penerimaan_pakan/transaksi/ganti_hand_pallet",
            data : {
                kode_flok : kode_flok,
                kode_barang : kode_barang,
                data_ke_detail_pakan : data_ke_detail_pakan,
...
function set_ganti_hand_pallet(elm, data_ke_detail, data_ke_detail_pakan){

    var berat = $(elm).find('td.ghp_berat').text();
    $('tr.tr-detail[data-ke="'+data_ke_detail+'"] tr.tr-detail-pakan[data-ke="'+data_ke_detail_pakan+'"] td.no-kavling').attr('data-berat-hand-pallet',berat);
    $('.bootbox button.btn-success').click();
    autoplus(data_ke_detail, data_ke_detail_pakan);
    get_detail_kandang('input.berat-pallet');
...
function autoplus(data_ke_detail, data_ke_detail_pakan){
    var data_berat_pallet = $('tr.tr-detail[data-ke="'+data_ke_detail+'"] tr.tr-detail-pakan[data-ke="'+data_ke_detail_pakan+'"] td.no-kavling').attr('data-berat-pallet');
    var data_berat_hand_pallet = $('tr.tr-detail[data-ke="'+data_ke_detail+'"] tr.tr-detail-pakan[data-ke="'+data_ke_detail_pakan+'"] td.no-kavling').attr('data-berat-hand-pallet');
    var berat = '';
    if(data_berat_pallet && data_berat_hand_pallet){
        berat = parseFloat(data_berat_pallet)+parseFloat(data_berat_hand_pallet);
    }
    $('tr.tr-detail[data-ke="'+data_ke_detail+'"] tr.tr-detail-pakan[data-ke="'+data_ke_detail_pakan+'"] td.berat-pallet input').val(berat);

}

function check_pallet(elm){
    var _pallet = $.trim($(elm).closest('tr').find('td.no-kavling').text());
    var _scan = $.trim($(elm).val());
    if(_pallet == _scan){
        konfirmasi_selesai(elm);
    }else{
        $(elm).next('span.help-block').text('Barcode yang discan tidak sesuai');
    }    
}

function showNopolImage(elm){
    var _nomerDo = $('#table-daftar-do-sj table>tbody>tr:first>td:first').text();
    var url = 'penerimaan_pakan/transaksi/showImage?do='+_nomerDo;
