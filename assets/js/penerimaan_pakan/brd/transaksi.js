    ...
    $.ajax({
        ...
        success : function(data) {
            if(data == 0){
            	...
            	get_data_surat_jalan(data.no_op,function(data_sj){
            		//console.log(data_sj);
            		var _no_sj;
                    var _nopol_kirim;
                    var _tanggal_sj;
                    var _kuantitas_kg;
                    var _kuantitas_zak;
                    var _tanggal_verifikasi_do;
            		
            		if(data.no_sj){
            			_no_sj = data.no_sj;
            		}
            		else{
            			_no_sj = (data_sj.status == 1) ? data_sj['content']['pjs'][0]['nomor_sj'] : 'N/A';
            		}
                    
                    if(data.nopol_kirim){
                        _nopol_kirim = data.nopol_kirim;
                    }
                    else{
                        _nopol_kirim = (data_sj.status == 1) ? data_sj['content']['pjs'][0]['nopol_kirim'] : 'N/A';
                    }
                    
                    if(data.tanggal_sj){
                        _tanggal_sj = data.tanggal_sj;
                    }
                    else{
                        _tanggal_sj = (data_sj.status == 1) ? data_sj['content']['pjs'][0]['tanggal_sj'] : 'N/A';
                    }
                    
                    if(data.kuantitas_kg){
                        _kuantitas_kg = data.kuantitas_kg;
                    }
                    else{
                        _kuantitas_kg = (data_sj.status == 1) ? data_sj['content']['pjs'][0]['kuantitas_kg'] : 'N/A';
                    }
                    
                    if(data.kuantitas_zak){
                        _kuantitas_zak = data.kuantitas_zak;
                    }
                    else{
                        _kuantitas_zak = (data_sj.status == 1) ? data_sj['content']['pjs'][0]['kuantitas_zak'] : 'N/A';
                    }
                    
                    if(data.tanggal_verifikasi_do){
                        _tanggal_verifikasi_do = data.tanggal_verifikasi_do;
                    }
                    else{
                        _tanggal_verifikasi_do = (data_sj.status == 1) ? data_sj['content']['pjs'][0]['tgl_verifikasi_do'] : 'N/A';
                    }
                    ...
                    $('#nomor-sj').text(_no_sj);
                    $('#tanggal-sj').text(_tanggal_sj);
                    $('#kuantitas-kg').text(_kuantitas_kg);
                    $('#kuantitas-zak').text(_kuantitas_zak);
                    $('#tanggal-verifikasi-do').text(_tanggal_verifikasi_do);
...
function detail_penimbangan_pakan(elm,_data_ke){
    var sub_data_ke;
    var data_ke;
