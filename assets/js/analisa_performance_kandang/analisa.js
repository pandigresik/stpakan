...
function get_data_detail_order(e) {
    ...
    if (kode_farm && tanggal_kebutuhan_awal && tanggal_kebutuhan_akhir) {
        $.ajax({
            type : "POST",
            url : "analisa_performance_kandang/main/baru",
            data : {
                kode_farm : kode_farm,
                no_order : no_order,
                status : status,
...
function selected(e) {
    ...
    if (tambah == 0) {
        ...
        $.ajax({
            type : "POST",
            url : "analisa_performance_kandang/main/tambah_daftar_barang",
            data : {
...
function tambah_barang() {
    $.ajax({
        type : "POST",
        url : "analisa_performance_kandang/main/tambah_barang",
        data : {
        },
        success : function(data) {
...
function simpan_baru() {
    ...
    _message += '<label class="text-center">Apakah anda yakin akan melanjutkan proses simpan data LHK dan Pakan ?</label>';
    _message += '</div>';
            ...
            success : {
                ...
                callback : function() {
                    ...
                    _ok.push({
                        ...
                        'tanggal_kebutuhan_awal' : $('#tanggal-kebutuhan-awal').val(),
                        'tanggal_kebutuhan_akhir' : $('#tanggal-kebutuhan-akhir').val(),
                    });
                    _params.push({
                    ...
                    cek_order_kandang(_params, function(result) {
                        if (result.ada == 0) {
                            ...
                            simpan_analisa_performance_kandang(_params1, function(result) {
                                if (result.success == 1) {
                                    $('#btn-release').removeAttr('disabled');
                                    $('#no-order').val(result.no_order);
                                    $('#status').val('D');
                                    generate();
                                    toastr.success('Simpan data LHK dan Pakan berhasil.', 'Berhasil');
                                } else {
                                    ...
                                    success : {
                                        ...
                                        callback : function() {
                                            ...
                                            if (blank == 0) {
                                                ...
                                                update_analisa_performance_kandang(_params2, function(result) {
                                                    if (result == 1) {
                                                        $('#btn-release').removeAttr('disabled');
                                                        generate();
                            ...
                            /*
                             * //console.log(blank+"dan"+data_ke); if(blank>0){
                             * box2.bind('hidden.bs.modal', function() {
                             * $('tr[data-ke="' + data_ke + '"]
                             * input.keterangan').select(); }); }
                             */
                        }
                    });
                }
...
function simpan_analisa_performance_kandang(data, callback) {
    if (data[0]['data_order_kandang'].length >= 1 && data[0]['data_order_kandang_d'].length >= 1 && data[0]['data_order_kandang_e'].length >= 1) {
        $.ajax({
            type : "POST",
            url : "analisa_performance_kandang/main/simpan_analisa_performance_kandang",
            data : {
                data : data
            },
            dataType : 'json',
...
function update_analisa_performance_kandang(data, callback) {
    // //console.log(data)
    if (data[0]['data_order_kandang_e'].length >= 1) {
        $.ajax({
            type : "POST",
            url : "analisa_performance_kandang/main/update_analisa_performance_kandang",
            data : {
                data : data
            },
            dataType : 'json',
...
function release() {
    ...
    if (_params.length >= 1) {
        ...
        $.ajax({
            type : "POST",
            url : "analisa_performance_kandang/main/release_analisa_performance_kandang",
            data : {
                data : _params
            },
            dataType : 'json',
            success : function(data) {
                if (data == 1) {
                    $('#status').val('N');
                    $('#btn-baru').attr('disabled', true);
                    $('#btn-release').attr('disabled', true);
                    toastr.success('Release data LHK dan Pakan berhasil.', 'Berhasil');
                } else if (data == 2) {
...
function generate() {
    ...
    if (kode_farm && tanggal_kebutuhan_awal && tanggal_kebutuhan_akhir && diff_tgl_keb_akhir <= 5 && diff_tgl_keb_awal >= 0) {
        ...
        ajax_generate(kode_farm, tanggal_kebutuhan_awal, tanggal_kebutuhan_akhir);
...
function ajax_generate(kode_farm, tanggal_kebutuhan_awal, tanggal_kebutuhan_akhir) {
    $.ajax({
        type : "POST",
        url : "analisa_performance_kandang/main/generate",
        data : {
            kode_farm : kode_farm,
            tanggal_kebutuhan_awal : tanggal_kebutuhan_awal,
            tanggal_kebutuhan_akhir : tanggal_kebutuhan_akhir
        ...
        success : function(data) {
            ...
            $("#btn-tambah-barang").removeClass('hide');
...
function cek_order_kandang(data, callback) {
    if (data.length == 1) {
        $.ajax({
            type : "POST",
            url : "analisa_performance_kandang/main/cek_order_kandang",
            data : {
                data : data
            },
            dataType : 'json',
