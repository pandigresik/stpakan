var Plotting = {
    ...
    list_order_pembelian_pakan: function(elm, status) {
        ...
        if (!_error) {
            ...
            $.ajax({
                ...
                url: 'permintaan_pakan_v2/pembelian_pakan/list_order',
                success: function(data) {
    ...
    detail_pp: function(elm) {
        ...
        $.ajax({
            ...
            url: 'permintaan_pakan_v2/pembelian_pakan/detail_pp',
            success: function(data) {
            ...
            var _bisa_plot = $('#div_list_order tbody').find('i.glyphicon-resize-horizontal').length;
            if (!_bisa_plot) {
                $('#sisaPlottingPP').text(0);
            } else {
                $('#sisaPlottingPP').closest('.row').remove();
            }
    ...
    simpan_do: function(elm) {
        ...
        $('#div_detail_pp').find('.box-kendaraan>input').not(':disabled').each(function() {
            ...
            _do[_no_op][_ekspedisi].push(_tmp);
            ...
            callback: function(result) {
                if (result) {
                    $.ajax({
                        ...
                        url: 'permintaan_pakan_v2/pembelian_pakan/simpan_do',
                        dataType: 'json',
                        success: function(data) {
                            bootbox.hideAll();
                            if (data.status) {
    ...
    do_pdf: function(no_do) {
        window.open('permintaan_pakan_v2/pembelian_pakan/do_pdf?no_do=' + no_do);
    },
    cetak_do: function(no_do, tgl_kirim, kode_ekspedisi) {
        $.ajax({
            type: 'post',
            data: { no_do: no_do, tgl_kirim: tgl_kirim, kode_ekspedisi: kode_ekspedisi },
            url: 'permintaan_pakan_v2/pembelian_pakan/detail_do',
            dataType: 'html',
            async: false,
            success: function(data) {
                var _options = {
