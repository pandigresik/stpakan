var Import = {
	...
	periksaDOCin : function(obj){
		...
		for(var _sn in obj){
			...
			for(var _d in _x){
				...
				_tmpTahun = _x[_d]['Siklus'].substring(0, 4);
				/* cari awal docin */
				if(empty(_awalDocin)){
	...
	validFormatKolom : {
		...
		'Farm' : /^\w+$/,'Kandang' : /^\d{2}$/,'Tanggal_Docin' : 'validDate','Siklus' : /^\d{4}\-\d{1}$/,'Populasi': /^\d+$/,'Tanggal_Panen' : 'validDate','Strain' : /^\w+$/
	},
	...
	periksaIsiBaris : function(baris){
		...
		for(var i in baris){
			...
			if(t == 'validDate'){
				...
				if(!t.test(baris[i])){
					_error++;
					_msg.push('Kolom '+i+': '+baris[i]+' tidak valid');
				}
			}
		}
	...
	simpanDocIn : function(){
		...
		console.log(Import.getPeriodeSiklus());
