(function($) {
  $.fn.scrollabletable = function(options){
     ...
     var _date = new Date();
     var _rand = _date.getTime();//Math.floor(Math.random() * 100);
     var _cur_table = this;
     ...
     $(_cur_thead).find('tr').each(function(){
          ...
          $(this).find('th').each(function(){
            ...
            _div = $('<div></div>')
                   ...
                   .css({
                    ...
                    'max-width' : $(this).width(), // tambahkan 2px supaya sama di chroome dan opera
                   //       'height': $(this).height()
                   });
            _th.html(_div);
            _th.appendTo(_tr);
      ...
      if(scrollHorizontal && settings.scroll_horizontal){
    	 ...
    	 */     
            ...
            var _display = ($(this).hasClass('hide')) ? 'none' : 'table-cell' ;
    	    	$(this).css({
    	    		      'display': _display,
    	              'vertical-align':'middle',
    	    ...
    	    $('#'+_rand+'left_section_virtual').css({
    	      ...
    	      'background-color': '#FFFFFF',
    	      'padding' : '0px',
    	      'max-height' : settings.max_height_scrollable + 'px',
    	      'overflow-y' : 'hidden',
    	    });
    	}
  	...
  	if(!scrollHorizontal && settings.scroll_horizontal){
  		$('#'+_rand+'div_header_virtual_kj').css({
  	        'max-width': $('#'+_rand+'table_wrapper_kj').innerWidth() -  settings.padding_right,
  	      });  
  	}
