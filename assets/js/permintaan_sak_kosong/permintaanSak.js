            ...
            bootbox.confirm({
                ...
                callback: function(result) {
                    if(result){
                        var url = 'permintaan_sak_kosong/permintaan/simpan';
                        var _prefix_ppsk = _form.find('input[name=sak_tersimpan]').data('prefix_ppsk');
                        var _data = {/*'kategori':_kategori, */'harga_sak' : _hrgJual,'jml_sak' : _sakdiminta, 'kode_budget' : _keterangan, 'user_peminta' : _userpeminta, 'no_ppsk': _prefix_ppsk, 'status':_nextStatus,'jml_over':_jmlOver,'no_do' : ''};
    ...
    editView : function(elm){
      ...
      $.ajax({
        ...
        success : function(data){
          ...
          if(hasil){
              var _form = $('#div_permintaan').find('form');
              var _kategori = $.trim(_form.find('input[name=kategori]:checked').val());
              if (_kategori == 'E') {
...
function hitungSisaBudget(str){
  var _val = str.value.replace('.','');
  var total_sak = $('#sak_tersimpan_t').val();
  var total_budget = $('#budget_total').val();
  var budget_sisa = $('#budget_sisa_t').val();

  $('#sak_tersimpan').val(total_sak - _val);
  $('#budget_sisa').val(budget_sisa - _val);

  if (budget_sisa - _val < 0) {
